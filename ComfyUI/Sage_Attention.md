# Correct Installation Steps for Sage Attention  
...
> Required for Sage Attention installation.
...
Extract the archive, then copy both the `include` and `libs` folders into:
...
Place the `.whl` file in the **ComfyUI root folder**, then run:
...
Add the following flag to the ComfyUI launch command:
...
Launch ComfyUI and check the console/log output.
